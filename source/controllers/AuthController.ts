...
import {
  UserActivationUUID,
  UserActivationUUIDModel
} from '../models/UserActivationUUID'
...
export default class AuthController extends Controller {
  ...
  public static async register(
    ...
    if (err) return res.status(400).json(this.error(err.message))
    // Hash password
    ...
    if (this.userModel.isInstance(user)) {
      ...
      const letter = pug.renderFile(
        path.resolve('public/letters/AccountCreated.pug'),
        {
          ...
          link:
            ...
            uuid,
          imgSrc:
            req.protocol + '://' + req.get('host') + 'public/images/dating.jpg'
        }
      )
  ...
  public static async login(req: Request, res: Response, next: NextFunction) {
    const user = await this.userModel.getOneWith({
    ...
    } else return res.status(404).json(this.error(user.message))
  }
  ...
  public static async verify(req: Request, res: Response, next: NextFunction) {
    ...
    if (
      userActivationUUIDModel.isInstance(userActivationUUID) &&
      this.userModel.isInstance(user)
    ) {
      if (
        userActivationUUID.user_id == userId &&
        userActivationUUID.uuid == req.params.uuid
      ) {
        await this.userModel.updateWhere({ id: userId }, { isVerified: true })
        res.statusCode = 200
